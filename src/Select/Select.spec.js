...
const SelectTest = class extends React.Component {
  ...
  render() {
    const ops = [{value: 0,  label: 'Select any option and click the button'}].concat(this.state.clicked ? options2 : options1);
    return (
      <div>
        <Select value={this.state.value || "0"} options={ops} onChange={this.handleChange}/>
...
describe('<Select />', () => {
  ...
  it('should have two options lists', () => {
    ...
    expect(wrapper.find('div[data-select-option]').children().length).to.equal(2);
  })
  ...
  it('should clear field when click over clear zone', () => {
    ...
    expect(wrapper.find('div[data-select-placeholder]').children().length).to.equal(1);
  })
  ...
  it('should reset field when search (field == empty && press backspace event)', (done) => {
    ...
    typing(input, 'a', () => {
      typing(input, '', () => {
        expect(wrapper.find('div[data-select-value-label]').children().length).to.equal(1);
        typing(input, '', {keyCode: KEY_BACKSPACE}, () => {
          expect(wrapper.find('div[data-select-value-label]').children().length).to.equal(0);
          expect(wrapper.find('div[data-select-placeholder]').children().length).to.equal(1);
          expect(onInputClearSpy).to.have.property('callCount', 1);
          expect(wrapper.find('div[data-select-menu-outer]').children().length).to.equal(0);
          done()
        })
      });
    });
  ...
  it('should open outer menu when KEY_DOWN is pressed', (done) => {
    ...
    const input = wrapper.find('input[data-select-input-search]').first();
    typing(input, '', {keyCode: KEY_DOWN}, () => {
      setTimeout(() => {
        wrapper.update()
        expect(wrapper.find('div[data-select-menu-outer]').children().length).to.equal(1);
        expect(wrapper.find('div[data-select-option]').children().length).to.equal(2);
        done()
      }, 2)
    })
  ...
  it('should open outer menu when KEY_DOWN is pressed and searchable is false', (done) => {
    ...
    const select = wrapper.find('div[data-select-multi-value-wrapper]').first()
  ...
  it('should render a custom value renderer', () => {
    ...
    wrapper.find('div[data-select-control]').at(0).simulate('mouseDown');
    wrapper.find('div[data-select-option]').at(0).simulate('mouseDown');
